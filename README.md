...
The v0.03 editor is Kinetic Studio: a fixed three-column workbench where the cast, motion map, live object, opening choreography and cut plan stay connected. Six cutwork voices can be selected for new compositions, every part can be selected in the object or inspector, and the same opening angle drives a continuous motion scrubber. The previous workshop remains available at `?legacy=1`; existing projects and geometry remain intact. See the [Kinetic Studio decision record](docs/architecture/0004-kinetic-choreography.md) and the [sculpture and editing contract](docs/architecture/0003-cut-paper-sculptures.md).
